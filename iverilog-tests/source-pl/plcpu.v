...
module PLCPU(
 ...
 // instantiation of pc unit
    assign NPC_raw = flush ? NPC_from_ex : (PC_out + 32'd4);
    assign NPC_hold = stall ? PC_out : NPC_raw;
    assign NPC = NPC_hold;
    PC U_PC(.clk(~clk), .rst(reset), .NPC(NPC), .PC(PC_out));
    NPC U_NPC(.PC(EX_pc), .NPCOp(EX_NPCOp), 
              .IMM(EX_immout), .RS1(fwdA), .NPC(NPC_from_ex));
    EXT U_EXT(
...
begin
    ...
    // load-use
    assign stall = EX_MemRead && (EX_rd != 5'b0) && ((EX_rd == rs1) || (EX_rd == rs2));

    //branch taken / jal / jalr in EX
    assign flush = EX_NPCOp[0] | EX_NPCOp[1] | EX_NPCOp[2];

    //data hazards
    assign fwdA_base = EX_RD1;
    assign fwdB_base = EX_RD2;
    ...
    assign mem_fwd_a = MEM_RegWrite && (MEM_rd != 5'b0) && (~MEM_MemRead) && (MEM_rd == EX_rs1);
    assign mem_fwd_b = MEM_RegWrite && (MEM_rd != 5'b0) && (~MEM_MemRead) && (MEM_rd == EX_rs2);
    assign wb_fwd_a = WB_RegWrite && (WB_rd != 5'b0) && (WB_rd == EX_rs1);
    assign wb_fwd_b = WB_RegWrite && (WB_rd != 5'b0) && (WB_rd == EX_rs2);
    assign fwdA = mem_fwd_a ? MEM_aluout : (wb_fwd_a  ? wb_wd : fwdA_base);
    assign fwdB = mem_fwd_b ? MEM_aluout : (wb_fwd_b  ? wb_wd : fwdB_base);
...
// MUX Gate
    ...
    //store
    always @(*) begin
        if (WB_RegWrite && (WB_rd != 5'b0) && (WB_rd == MEM_rs2) && MEM_MemWrite)
            memdata_wr <= wb_wd;
        else
            memdata_wr <= MEM_RD2;
    end
